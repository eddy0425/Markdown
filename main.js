document.addEventListener('DOMContentLoaded', function() {
    ...
    if (editorToolbar && toolbarContainer && modeSelector) {
        // 默认隐藏工具栏，切换到编辑模式时才显示
        editorToolbar.style.display = 'none';
        
        // 移除工具栏的原始位置
        if (editorToolbar.parentNode) {
            editorToolbar.parentNode.removeChild(editorToolbar);
        }
        
        // 插入到模式选择器后面
        modeSelector.parentNode.insertBefore(editorToolbar, modeSelector.nextSibling);
    }
    ...
    // 从本地存储加载上次选择的模式
    const lastMode = localStorage.getItem('md-view-mode');
    if (lastMode) {
        modeSelector.value = lastMode;
    ...
    function setViewMode(mode) {
        ...
        if (mode === 'read') {
            // 阅读模式：隐藏编辑器，全屏显示内容，显示字体大小控制和文件选择按钮，隐藏保存按钮
            editorContainer.classList.add('hidden');
            markdownContainer.classList.add('full-width');
            ...
            // 编辑模式：显示编辑器和预览，分栏布局，隐藏字体大小控制和文件选择按钮，显示保存按钮
            editorContainer.classList.remove('hidden');
            markdownContainer.classList.remove('full-width');
            fontSizeControls.style.display = 'none';
            fileInputContainer.style.display = 'none';
            saveMarkdownBtn.style.display = 'block';
            ...
            // 将编辑器工具栏移动到更合适的位置
            if (toolbarContainer && editorToolbar && !toolbarContainer.contains(editorToolbar)) {
                // 将工具栏插入到模式选择器之后
                const modeSelector = document.getElementById('mode-selector');
    ...
    function initEditorContent() {
        ...
        const savedContent = localStorage.getItem('smde_markdown-editor');
        ...
        fetch('example.md')
            ...
            .catch(error => {
                console.error('无法加载示例Markdown文件', error);
                const defaultContent = '# 欢迎使用Markdown阅读器\n\n这是一个简单的Markdown阅读器，支持以下功能：\n\n- 阅读模式：全屏显示内容\n- 编辑模式：分栏编辑和预览\n- 代码高亮\n- 主题切换\n\n## 如何使用\n\n1. 选择上方的模式（阅读/编辑）\n2. 在编辑模式下，左侧可编辑，右侧实时预览';
...
function initTheme() {
    // 从本地存储加载主题设置
    const darkMode = localStorage.getItem('md-dark-mode') === 'true';
